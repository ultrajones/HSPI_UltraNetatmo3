...
Public Class HSPI
  ...
  '
  Public WeatherStationThread As Thread
  Public HomeCoachThread As Thread
...
#Region "HSPI - Base Plugin API"
          ...
          Case "Netatmo State"
            ...
            Select Case dev_value
              Case -1
                ...
                WriteMessage("Netatmo checking is enabled.", MessageType.Informational)
              ...
              Case -2
                ...
                WriteMessage("Netatmo checking is disabled.", MessageType.Informational)
  ...
  Public Function InitIO(ByVal port As String) As String Implements HomeSeerAPI.IPlugInAPI.InitIO
    ...
    Try
      ...
      '
      gAPIClientId = GetSetting("API", "ClientId", gAPIClientId)
      gAPIClientSecret = GetSetting("API", "ClientSecret", gAPIClientSecret)
      gAPIUsername = GetSetting("API", "Username", "")
      gAPIPassword = GetSetting("API", "Password", "")
      gAPIGetFavorites = CBool(GetSetting("API", "GetFavorites", False))

      '
      ' Refresh the Access Token
      '
      NetatmoAPI = New hspi_netatmo_api
      ...
      If bDBInitialized = True Then
        ...
        '
        WeatherStationThread = New Thread(New ThreadStart(AddressOf CheckWeatherStationThread))
        WeatherStationThread.Name = "WeatherStationThread"
        WeatherStationThread.Start()

        WriteMessage(String.Format("{0} Thread Started", WeatherStationThread.Name), MessageType.Debug)

        '
        ' Start the process command queue thread
        '
        HomeCoachThread = New Thread(New ThreadStart(AddressOf CheckHomeCoachThread))
        HomeCoachThread.Name = "HomeCoachThread"
        HomeCoachThread.Start()

        WriteMessage(String.Format("{0} Thread Started", HomeCoachThread.Name), MessageType.Debug)
  ...
  Public Sub ShutdownIO() Implements HomeSeerAPI.IPlugInAPI.ShutdownIO
    ...
    Try
      ...
      If WeatherStationThread.IsAlive = True Then
        WeatherStationThread.Abort()
      End If

      If HomeCoachThread.IsAlive = True Then
        HomeCoachThread.Abort()
      End If
...
#Region "HSPI - Public Sub/Functions"
  ...
  Public Function ValidAPIToken() As Boolean
    ...
    Try
      ...
      Return NetatmoAPI.ValidAPIToken()
  ...
  Public ReadOnly Property GetStatistics(ByVal StatisticsType As String) As Integer
    Get
      Select Case StatisticsType
        Case "APISuccess"
          Return NetatmoAPI.QuerySuccessCount()
        Case "APIFailure"
          Return NetatmoAPI.QueryFailureCount()
        Case Else
          Return 0
      End Select
    End Get
